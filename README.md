...
Performance of the pipeline was evaluated on **1,022 microscopy images** and compared in three scenarios (Fig. 5). **Left:** the ResNet18-based cell count classifier alone achieved 99.9% accuracy, demonstrating highly reliable prediction of the number of embryonic cells. **Center:** the slot-based segmentation network alone reached 96.97% counting accuracy, with most errors caused by over-segmentation of two-cell embryos. **Right:** combining the count classifier with the segmentation network by selecting the top-K predicted masks according to the estimated cell count increased the overall detection accuracy to 99.61%. Finally, each predicted segmentation mask was approximated by a best-fitting ellipse to obtain a compact geometric representation of the detected cell (Fig. 6). Since ground-truth segmentation masks were not available for these images, the reported **mean Dice coefficient (0.981)** and **mean IoU (0.964)** quantify the agreement between the predicted segmentation masks and their corresponding fitted ellipse masks, rather than segmentation accuracy with respect to biological ground truth. These results demonstrate that ellipse fitting preserves the shape of the predicted cell masks with very high fidelity while providing a compact and interpretable representation suitable for quantitative analysis.
...
### Image filenames naming convention

**`<EmbryoID>_<Time>_FOC_<CellCount>.tif`**

For example, **`001_000h20m_FOC_1.tif`** denotes:

* **001** – embryo identifier,
* **000h20m** – time elapsed since the beginning of the time-lapse acquisition (0 hours 20 minutes),
* **FOC** – all-in-focus image generated by focus stacking from three focal planes,
* **1** – manually assigned **ground-truth** cell count (zygote stage), used only **for training and evaluation**; can be 1, 2, 4.

### ZIP archives

The link to repository below includes the following supporting files:

* **`environment.yml`** – Conda environment specification listing all required Python packages and their versions to reproduce the software environment.
